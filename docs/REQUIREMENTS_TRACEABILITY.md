...
| EV-01 | Runtime evidence is signed/hash-pinned and offline | release manifest and cardiometabolic pack; Ed25519 manifest signature support (`scripts/sign_release_manifest.py`, `verify_release_manifest.py`) | artifact-integrity test; signing roundtrip + tamper/rotation drills in `tests/test_release_signing.py` | Preclinical control passed locally | Provenance, licensing and independent review register incomplete |
...
| PS-01 | No plaintext PHI in relay/push/log surfaces | opaque relay schema; fixed generic push constant; Pydantic `extra="forbid"` envelope schemas | `tests/test_privacy_surface.py` sentinel audit (responses, logs, push payloads) | Preclinical control passed locally | Privacy + security | No independent penetration test or deployed-config capture |
