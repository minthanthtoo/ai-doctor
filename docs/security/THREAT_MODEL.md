...
| T-06 | Prompt/document injection alters authority | Data/instruction separation, strict candidate schema, no arbitrary tools; `extra="forbid"` rejects smuggled authority fields | Broker validation tests; `tests/test_broker_adversarial.py` injection/inertness drills | Implemented (local evidence) | Parser sandbox and provider-drift probes |
| T-07 | Model invents facts, treatment or urgency | Candidate-only schema, grounding checks, prohibited language, deterministic fallback; urgency fields absent from augmentation output; dedupe preserves deterministic hypotheses; schema length caps bound hostile output | `tests/test_broker_adversarial.py`: overflow, type-bomb, string-bomb, urgency-immutability drills | Implemented (local evidence) | Task-specific qualification and provider-drift probes |
| T-08 | Stale or corrupted clinical release is used | Hash-pinned artifact, stable channel fails closed while unapproved; Ed25519-signed manifest with fail-closed startup verification and required-artifact re-hash | `tests/test_release_signing.py`: tamper, wrong-key, rotation-revocation, require-signed and digest-consistency drills | Implemented (local evidence) | Signed manifest expiry policy and deployed-configuration drill |
